...
ax.plot(x, np.sin(x - 3), color='#FFDDAA', linestyle='dashed') # -
...
plt.subplot(3, 1, 3)
plt.plot(x, np.sin(x), '-k', label='s\sin x$')
...
plt.plot(x, y, "-k")
plt.errorbar(x, y, yer=dy, fmt=".r")
plt.fill_between(x, y - dy, y + dy, color="0.75", alpha=0.5)
...
# plt.contourf(X, Y, Z, 10, cmap="RdGyr")
plt.imshow(
    Z,
    extent=[0, 6, 0, 5],
    origin="lower",
