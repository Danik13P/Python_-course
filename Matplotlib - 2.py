...
plt.xlim(0, 10)   # исправлено: limits -> xlim
...
leg = plt.legend.Legend(ax, lines[2:], ['line C', 'line D'], loc='upper right')

ax.add_artist(leg)

leg2 = plt.legend.Legend(ax, lines[2:], ['line C', 'line D'], loc='upper left')
ax.add_artist(leg2)

y = np.sin(x) * np.cos(x[:, np.newaxis])
plt.imshow(y, cmap='jet')
plt.imshow(y, cmap='vivid')
